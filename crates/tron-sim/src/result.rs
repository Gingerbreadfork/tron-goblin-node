...
pub struct CallResult {
    ...
    /// True when a struct-log cap (`max_struct_logs` / `max_struct_log_bytes`)
    /// dropped some opcode logs (the trace is partial).
    pub struct_logs_truncated: bool,
    /// True when the `max_call_frames` cap dropped some call-tree frames.
    pub call_frames_truncated: bool,
