...
pub trait JournalTr {
    ...
    /// Decrements the balance of the account, returning `Ok(false)`
    /// if the account had insufficient funds (no mutation happens in
    /// that case). Used by the TRON-fork state-mutating opcode
    /// bridges (`FREEZE` / `FREEZEBALANCEV2`) to reflect the
    /// on-stake balance debit in revm's journaled view.
    fn balance_decr(
        &mut self,
        address: Address,
        balance: U256,
