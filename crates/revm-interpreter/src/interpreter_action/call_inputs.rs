...
pub struct CallInputs {
    ...
    // ====================================================================
    // TRON fork extension (CALLTOKEN opcode 0xd0).
    // ====================================================================
    //
    // These two fields carry the TRC-10 transfer parameters across a
    // CALLTOKEN. They default to 0 for all standard EVM calls (CALL,
    // CALLCODE, DELEGATECALL, STATICCALL), so an Ethereum-only consumer
    // of this fork sees no observable difference.
    //
    // The TRC-10 transfer side effect (debit caller, credit target in
    // `Account.asset_v2`) is performed by the host *outside* the
    // interpreter — see `tron-tvm`'s precompile/inspector glue. The
    // values here are the consensus-critical inputs to that transfer
    // and the values exposed to the callee via the CALLTOKENVALUE /
    // CALLTOKENID opcodes (`0xd2` / `0xd3`).
    /// TRC-10 token id (zero when the call wasn't a CALLTOKEN).
