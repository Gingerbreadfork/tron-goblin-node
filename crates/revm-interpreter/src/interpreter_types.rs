...
pub trait InputsTr {
    ...
    /// **TRON-fork extension.** TRC-10 token id passed by the parent
    /// CALLTOKEN frame. Defaults to `0` for standard CALL / CREATE /
    /// top-of-tx entry — same value the standard `CALLTOKENID` opcode
    /// (`0xd3`) returns.
