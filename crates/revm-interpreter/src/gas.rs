...
pub struct Gas {
    ...
    memory: MemoryGas,
    /// **TRON fork extension** — per-contract dynamic-energy factor
    /// applied to every gas charge. `0` (the default) means "no
    /// penalty" — behaviour is identical to upstream revm. Non-zero
    /// values come from java-tron's `ALLOW_DYNAMIC_ENERGY` model where
    /// each contract has its own factor stored in `ContractStateStore`.
    ///
...
impl Gas {
    ...
    /// **TRON fork** — set the per-contract dynamic-energy factor for
    /// this frame. Should be called once when the frame starts (in
    /// `revm-handler::frame::call`). Subsequent gas charges via
    /// `record_cost_unsafe`, `record_regular_cost`, `record_state_cost`
    /// will be multiplied by `(DECIMAL + factor) / DECIMAL`.
    #[inline]
    pub const fn set_tron_dynamic_factor(&mut self, factor: i64) {
        self.tron_dynamic_factor = factor;
    }

    /// **TRON fork** — read the per-contract dynamic-energy factor.
    #[inline]
