...
mod tests {
    ...
    use context_interface::TronCompat;
    use database::{CacheDB, EmptyDB};
    use primitives::{address, eip3860, eip7954, hardfork::SpecId, Bytes, TxKind, B256};
    use state::{AccountInfo, Bytecode};

    // TRON fork — wrap CacheDB with TronCompat so our Host bound
    // (`DB: TronDatabaseExt`) is satisfied. CacheDB lives in
    // revm-database, above us in the dep graph, so we can't impl
    // TronDatabaseExt for it directly without forking that crate too.
    ...
    fn deploy_contract(
        ...
        let ctx = Context::mainnet()
            ...
            .with_db(TronCompat(CacheDB::<EmptyDB>::default()));
    ...
    fn test_transact_many_with_transaction_index_error() {
        ...
        let ctx = Context::mainnet().with_db(TronCompat(CacheDB::<EmptyDB>::default()));
        let mut evm = ctx.build_mainnet();
    ...
    fn test_transact_many_success() {
        ...
        evm.db_mut().0.insert_account_info(
            caller,
            AccountInfo::new(
